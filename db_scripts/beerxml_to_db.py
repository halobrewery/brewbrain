...
def read_recipe(session, recipe):
  ...
  if existing_style == None:
    ...
    if existing_style == None:
      existing_style = session.scalars(select(Style).filter(Style.name.ilike(style.name))).first()
      if existing_style == None:
        # ... couldn't find the style
        raise ValueError(f"Failed to find style '{style.name}' for recipe: {filepath}")
  ...
  if num_mash_steps > MAX_MASH_STEPS:
    raise ValueError(f"Too many mash steps ({num_mash_steps}), in recipe: {filepath}")
  ...
  for hop in recipe.hops:
    ...
    if existing_hop == None:
      raise ValueError(f"Failed to find hop '{hop.name}', in recipe: {filepath}")
  ...
  for fermentable in recipe.fermentables:
    ...
    if f_type == 'grain':
      ...
      if existing_grain == None:
        raise ValueError(f"Failed to find grain '{f_name}', in recipe: {filepath}")
      ...
      if existing_adjunct == None:
        raise ValueError(f"Failed to find adjunct '{f_name}, in recipe: {filepath}")
  ...
  for microorganism in recipe.yeasts:
    ...
    if existing_microorganism == None:
      raise ValueError(f"Failed to find microorganism '{m_name}, in recipe: {filepath}")
  ...
  # Need to call flush if any deletes happened to mark all associated cascades as deleted too!
  session.flush()
...
if __name__ == "__main__":
  ...
  def session_read_beerxml_files():
    ...
    with Session(engine) as session:
      for filepath in glob.glob(os.path.join(datapath, "beersmith_recipes*.xml")):
        try:
          recipes = beerxml_parser.parse(filepath)
        except:
          print(f"Erroneous file found ({filepath}), exiting.")
          session.rollback()
          return

        for recipe in recipes:
          try:
            read_recipe(session, recipe)
          except:
            print("Failed to read recipes, rolling back and exiting.")
            session.rollback()
            return
