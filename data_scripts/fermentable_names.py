...
def match_fermentable_id(fermentable_name, fermentable_name_to_id):
  ...
  f_name = re.sub(r"\s*(english|british|german|lund)\s*", "", f_name, flags=re.IGNORECASE)
  ...
  f_name = re.sub(r"\-*\s*\(?(sugar equivalent|franco\s*belges?|malt=|bob'?s\s*mills|clear\s*choice|muntons|homemade|malting|cal\s*rose|cargill|((green|black|gold)\s?)?swaen|munton'?s?|warminster|biscoff|patagonia|global malt|(thomas)?\s*fawcett?|crisp|best\s*(malz|maltz)|simpson'?s?|valley\s*(malt)?|briess|bairds|gladfield'?s?|château|chateau|castle|dingemans|brewers\s+malt|rahr|organic|dehusked|weyermann?)\)?\s*\-*\s*", " ", f_name, flags=re.IGNORECASE)
